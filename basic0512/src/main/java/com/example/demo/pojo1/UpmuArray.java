...
public class UpmuArray {
	...
	public static void main(String[] args) {
		...
		int endList = noticeList.lastIndexOf(".");//18
		int endDetail = noticeDetail.lastIndexOf(".");//20
		int endInsert = noticeInsert.lastIndexOf(".");//20
		int endUpdate = noticeUpdate.lastIndexOf(".");
		int endDelete = noticeDelete.lastIndexOf(".");
		System.out.println(endList);//18
		noticeList = noticeList.substring(1,endList);//1은 슬래쉬날리고 뒤에 18은 .pj1을 날리기 위함
		...
		//substring 문자열을 자를때 사용한다 - 자르는 구간이 필요함
		noticeInsert = noticeInsert.substring(1,endInsert);//1, 18
