...
//카카오로그인 버튼 클릭 -> https://kauth.kakao.com/oauth/authorize?client_id=REST ful 키&redirect_uri=http://localhost:9000/auth/kakao/callback&response_type=code"
public class KakaoController {
	...
	public String kakaoCallback(HttpServletRequest req, HttpServletResponse res) {
		...
		//카카오 로그인 클릭시 카카오 서버에 요청(응답화면도 카카오서버가제공함)을 하고 요청에 대한 응답
		//코드를 받아옴
		...
		//카카오서버측에서 응답으로 보내주는 토큰값 읽어오기
		BufferedReader br = null;//filter스트림 클래스이고 네트워크 요청에 대한 응답을 읽기위해
		...
		try {
			...
			pMap.put("client_id", "a2003af7efd216db91c88af72684a992");
			pMap.put("redirect_uri", "http://localhost:9000/auth/kakao/callback");
			pMap.put("code", code);
			//위에서 정의한 정보들을 POST방식으로 헤더에 담기
			StringBuilder postData = new StringBuilder();
			for(Map.Entry<String, Object> params: pMap.entrySet()) {
			...
			//POST방식으로 사용자가 입력한 값을 넘길 때 추가되는 설정임 - postman
			conn.setRequestProperty("Content-type", "application/x-www-form-urlencoded;charset=utf-8");
			//보내는 데이터 길이 -  Tomcat에서 카카오서버측에 보내는 메시지의 길이계산
			...
			//만일 BufferedReader 를 사용하지 않으면 read메소드만 사용가능한데 BufferedReader를 사용했기에
			...
			while((inputLine = br.readLine()) !=null) {
				sb.append(inputLine);
			}//end of while - 네트워크 통해서 넘어오는 패킷을 읽어서 StringBuilder에 담기
			String jsonStr = sb.toString();
			//아래서 출력되는 값은 인증코드를 POST방식으로 카카오측에 요청하면 응답으로 보내주는 토큰값을 받기 위해서 작성함
			...
			//카카오 서버에서 넘겨준 토큰을  가지고 카카오 로그인 서버측에 프로필을 받아오는 URl 다시 요청해야 함
			//그래서 미리 확인해 본다
			...
			//토큰을 넘겨서 세 번째 요청{카카오 등록된 프로필정보를 받아오는}을 함
			br = null;
			//아래에서 다시 생성하면 기존에 참조하던 객체는 Candidate상태로 빠짐(넌 쓰레기값)
			...
			//아래서 출력되는 값은 토큰을 POST방식으로 카카오측에 요청하면 응답으로 보내주는 개인 프로필을 받기위함 
		...
		try {
			res.sendRedirect("/gym.jsp");//반드시 예외처리를 해줘야한다
		} catch (Exception e) {
