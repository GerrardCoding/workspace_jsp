...
public interface Controller3 {
	...
	//자유게시판일때
	//인터페이스는 추상메소드만 가질 수 있어서 abstract는 생략이 가능함
	public abstract Object boardList(HttpServletRequest req, HttpServletResponse res);
}
/*
pj1과 pj2에서는 없었던 ModelAndView 추가하였다
ModelAndView는 목록화면을 지원하기 위해 설계되었다.
jsp 페이지의 배포 위치는 WEB-INF\views아래이다
객체가 유지되는 scope는 request이다.

XXXController클래스도 메소드 단위로 나누어서 구현해 본다
pj1과 pj2에서는 NoticeController또는 BoardController는
if문으로 분기하였다
직관적이지 않아서 코드 분석이 별로이다.
