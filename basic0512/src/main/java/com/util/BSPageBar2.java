...
public class BSPageBar2 {
	...
	public void setPageBar() {
		//문자열을 조작할 때 원본이 바뀌지 않는 String클래스 대신에 StringBuilder{싱글스레드안전- 속도는 조금 빠름}나 StringBuffer{멀티스레드안전- 속도는 조금 느림}클래스를 
		...
		if(totalRecord>0) {
			...
			for(int i=0;i<pagePerBlock;i++) { //전체 페이지 블록 수는 3개이다 1 2 3 >>  << 4 5 6 >> << 7
				...
				if(nowBlock*pagePerBlock+i==nowPage) {
					//현재 내가 보고있는 2번째 페이지블록에서 5번 페이지를 보고 있다면 5번 숫자에 대해서는 링크가 필요없다
			...
			if(totalBlock > nowBlock+1) {
				...
				pageLink.append("<span aria-hidden=\"true\">&raquo;</span>"); //&laquo << 겹쳐진 화살표 표시
				pageLink.append("</a>");	
				pageLink.append("</li>");
			}
		}
		logger.info("pageLink.toString():"+pageLink.toString());
		pagination = pageLink.toString(); //toString()을 붙인 이유는 pageLink가 StringBuilder타입이니까 _ 그래서
	}
	//getter메소드 선언
	//
	public String getPageBar() { //getPageBar()의 리턴타입이 String이니까 print메소드 안에서 호출이 가능하다
		this.setPageBar();
		//그때마다 달라져야 한다. 인스턴스화 할때마다 달라져야 한다 -> BSPageBar pb = new BSPageBar();
