...
<%
	...
	//n건이 조회되면 안된다(pk가 적용되지 않았다 - 그래서 n건이 조회된다)
	//왜 n건이 나올 수 있는걸까?
	//myBatis는 동적(if문, when구문, choose문)쿼리를 지원하니까
    int size = 0;//지변이니까 초기화를 생략하면 에러발생함.
    //jsp에서는 오라클 연동을 직접 하지 않는다 - 왜냐면 인스턴스화를 할 수 없다.
    //ActionServlet을 경유해서 BoardController에서 forward를 통해서
    //요청객체에 담긴 정보를 꺼내서 쓴다
    ...
    if(bList !=null && bList.size()>0) {
		...
		//insert here - 조회 결과가 있을 때만 출력됨
		out.print(rmap);//{키=값, 키1=값1, 키2=값2,......}
    }
    //insert here - 조회 결과가 없으면 null인 상황
   // out.print(bList);브라우저에 쓰기 - 서버사이드 결정된 문자열 출력 - 데이터셋 역할(JSON포멧 - Gson API, 단순문자열포맷)
    //rmap = bList.get(0);//null,{}
   // out.print(rmap);//{} 깡통이다
    //List<Map<>> -> [{},{},{},....] -> JSON -> IOS, 안드로이드 서비스 가능함, Vue.js, ReactJS
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
           ...
           <!-- hidden속성은 화면없이 서버측에 값을 넘길때 사용할수 있다 -->
