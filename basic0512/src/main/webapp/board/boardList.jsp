...
<%
	...
	//req.setAttribute("bList"); -> BoardController에서 작성할 것 //get은 불러올 때, set은 담을 때
	List<Map<String,Object>> bList = (List<Map<String,Object>>)request.getAttribute("bList");
	//bList가 null이 아니기 위해서는 테스트 시나리오를 어떻게 가져가야 할까?
	//경우의 수 : 한 건도 등록되지 않았을 때 - 0 -myBatis에서 간섭(인터셉트) bList = new ArrayList<>();
	if(bList !=null){
		size = bList.size();	//4
	}
	 int numPerPage = 5;
	 ...
	 //out.print(mList);
	 //out.print(bList);//[{},{},{}] -> JSON 포맷 -> 하이브리드앱서비스가능하니까...
%>
<!DOCTYPE html>
<html>
<head>
...
%>            
                 <tr>
                     ...
                     <th><a href="javascript:fileDown('<%=rmap.get("B_FILE")%>')"><%=rmap.get("B_FILE") %></a></th>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
           <form id="f_board" method="post" enctype="multipart/form-data" action="./boardInsert.pj2"><!-- enctype넣을땐 method가 반드시 post!! -->
             <input type="hidden" name="method" value="boardInsert">
              ...
              <div class="input-group mb-3">
			  	<input type="file" class="form-control" id="b_file" name="b_file"><!-- id는 프론트 name은 서블릿 프론트 -->
